...
class Processor:
    ...
    def gen_install_script(log_path, pre_run, post_run, finals):
        ...
        with tempfile.NamedTemporaryFile(mode="w", delete=False) as f:
            f.write("#!/bin/sh\n")
            f.write("# This file was created by the Yoyo Installer.\n")
            ...
            if "YOYO_FAKE" in os.environ:
                logger.info("YOYO_FAKE is set, skipping the installation process.")
                f.write(
                    "echo 'YOYO_FAKE is set, skipping the installation process.'\n"
                )
            ...
            if "YOYO_SKIP_INSTALL" not in os.environ:
                for arg in arguments:
                    f.write(arg + " ")

            if "YOYO_SKIP_POSTINSTALL" not in os.environ:
                f.write("\n")
